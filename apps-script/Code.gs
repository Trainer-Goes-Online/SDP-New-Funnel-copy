/**
 * SDP CRM — downstream Meta CAPI event firing engine.
 *
 * Bound to the SDP CRM Google Sheet. On every USER edit to a trigger column
 * (call_booked / call_showed / sale_closed) this script reads the lead's row,
 ...
 * proxy through our Next.js backend). All secrets live in Apps Script
 * PropertiesService. This means a new client can be onboarded in ~15 min by
 * copy-pasting this file into their Sheet's Apps Script editor and setting
 * three script properties — no backend deploy required.
 *
 * Source of truth schema: 36-column row, A..AJ, as defined in apps-script/README.md.
 */
...
const COL = {
  ...
  // --- Manual / Apps Script-written ---
  CALL_BOOKED:               24,  // X
  BOOKING_TIME:              25,  // Y
  SCHEDULE_CAPI_EVENT_ID:    26,  // Z
  SCHEDULE_CAPI_SENT:        27,  // AA
  CALL_SHOWED:               28,  // AB
  SHOWUP_TIME:               29,  // AC
  SHOWUP_CAPI_EVENT_ID:      30,  // AD
  SHOWUP_CAPI_SENT:          31,  // AE
  SALE_CLOSED:               32,  // AF
  CONTRACTED_VALUE:          33,  // AG
  SALES_TIME:                34,  // AH
  HTSALE_CAPI_EVENT_ID:      35,  // AI
  HTSALE_CAPI_SENT:          36,  // AJ
};

// One entry per downstream event. The triggerCol checkbox flipping to TRUE
// is what causes the event to fire. valueCol is only relevant for the HT sale.
const EVENTS = {
  CALL_BOOKED: {
    eventName: 'CallBookedWithSDP',
    triggerCol: COL.CALL_BOOKED,
    timeCol: COL.BOOKING_TIME,
    eventIdCol: COL.SCHEDULE_CAPI_EVENT_ID,
    sentCol: COL.SCHEDULE_CAPI_SENT,
    eventIdSuffix: 'schedule',
    includeValue: false,
  },
  CALL_SHOWED: {
    eventName: 'CallDoneWithSDP',
    triggerCol: COL.CALL_SHOWED,
    timeCol: COL.SHOWUP_TIME,
    eventIdCol: COL.SHOWUP_CAPI_EVENT_ID,
    sentCol: COL.SHOWUP_CAPI_SENT,
    eventIdSuffix: 'showup',
    includeValue: false,
  },
  SALE_CLOSED: {
    eventName: 'SDPHighTicketPurchase',
...
function onSheetEdit(e) {
  try {
    ...
    // Only fire when the checkbox flips TO true.
...
function replayPendingEvents() {
  ...
  SpreadsheetApp.getActive().toast('Replayed ' + replayed + ' pending event(s)', 'SDP CRM', 5);
}
